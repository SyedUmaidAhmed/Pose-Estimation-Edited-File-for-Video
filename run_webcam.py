...
if __name__ == '__main__':
    ...
    while True:
        ...
        if mode == 1:
            ...
            shoulder_hand_dst_l = int( euclidian(find_point(pose, 5), find_point(pose, 7)))
            shoulder_hand_dst_r = int( euclidian(find_point(pose, 2), find_point(pose, 4)))
            ...
            head_ankle 			= int( euclidian(find_point(pose, 15), find_point(pose, 13)))
            hand_hand 			=  shoulder_hand_dst_l + shoulder_hand_dst_l + int( euclidian(find_point(pose, 2), find_point(pose, 5)))
